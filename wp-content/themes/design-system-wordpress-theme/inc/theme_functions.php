 ...
 */
function cagov_get_option( $option ) {
	$defaults = cagov_get_defaults();
...
if ( ! function_exists( 'cagov_get_layout' ) ) {
	/**
	 ...
	 */
	function cagov_get_layout() {
		$layout = cagov_get_option( 'layout_setting' );

		if ( is_single() ) {
			$layout = cagov_get_option( 'single_layout_setting' );
		}
		...
		if ( is_home() || is_archive() || is_search() || is_tax() ) {
			$layout = cagov_get_option( 'blog_layout_setting' );
		}

		return apply_filters( 'cagov_sidebar_layout', $layout );
	}
}

if ( ! function_exists( 'cagov_get_footer_widgets' ) ) {
	/**
	 ...
	 */
	function cagov_get_footer_widgets() {
		$widgets = cagov_get_option( 'footer_widget_setting' );
		...
		return apply_filters( 'cagov_footer_widgets', $widgets );
	}
}

if ( ! function_exists( 'cagov_show_excerpt' ) ) {
	/**
	 * Figure out if we should show the blog excerpts or full posts
	 *
	 * @since 1.3.15
	 */
	function cagov_show_excerpt() {
		global $post;

		// Check to see if the more tag is being used.
		$more_tag = apply_filters( 'cagov_more_tag', strpos( $post->post_content, '<!--more-->' ) );

		$format = ( false !== get_post_format() ) ? get_post_format() : 'standard';

		$show_excerpt = ( 'excerpt' === cagov_get_option( 'post_content' ) ) ? true : false;
		...
		return apply_filters( 'cagov_show_excerpt', $show_excerpt );
	}
}

if ( ! function_exists( 'cagov_show_title' ) ) {
	/**
	 ...
	 */
	function cagov_show_title() {
		return apply_filters( 'cagov_show_title', true );
	}
}
 ...
 * @since 3.0.0
 */
function cagov_show_entry_header() {
	$show_entry_header      = true;
	$show_title             = cagov_show_title();
	$has_before_entry_title = has_action( 'cagov_before_entry_title' );
	$has_after_entry_title  = has_action( 'cagov_after_entry_title' );

	if ( is_page() ) {
		$has_before_entry_title = has_action( 'cagov_before_page_title' );
		$has_after_entry_title  = has_action( 'cagov_after_page_title' );
	}
	...
	return apply_filters( 'cagov_show_entry_header', $show_entry_header );
}

if ( ! function_exists( 'cagov_get_premium_url' ) ) {
	/**
	 ...
	 */
	function cagov_get_premium_url( $url = 'https://generatepress.com/premium', $trailing_slash = true ) {
		if ( $trailing_slash ) {
			$url = trailingslashit( $url );
		}

		$args = apply_filters(
			'cagov_premium_url_args',
			array(
				'ref' => null,
				'campaign' => null,
			)
...
if ( ! function_exists( 'cagov_padding_css' ) ) {
	/**
	 ...
	 */
	function cagov_padding_css( $top, $right, $bottom, $left ) {
...
if ( ! function_exists( 'cagov_get_link_url' ) ) {
	/**
	 * Return the post URL.
	 *
...
if ( ! function_exists( 'cagov_get_navigation_location' ) ) {
	/**
	 ...
	 */
	function cagov_get_navigation_location() {
		return apply_filters( 'cagov_navigation_location', cagov_get_option( 'nav_position_setting' ) );
	}
}
 ...
 * @since 2.3
 */
function cagov_has_logo_site_branding() {
	$has_site_title = ! cagov_get_option( 'hide_title' ) && get_bloginfo( 'title' );
	$has_site_tagline = ! cagov_get_option( 'hide_tagline' ) && get_bloginfo( 'description' );
 ...
 */
function cagov_get_svg_icon( $icon, $replace = false ) {
	if ( 'svg' !== cagov_get_option( 'icons' ) ) {
		return;
	}
	...
	$output = apply_filters( 'cagov_svg_icon_element', $output, $icon );
	...
	return apply_filters( 'cagov_svg_icon', $output, $icon );
}
 ...
 */
function cagov_do_svg_icon( $icon, $replace = false ) {
	echo cagov_get_svg_icon( $icon, $replace ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in function.
}
 ...
 */
function cagov_get_media_query( $name ) {
	$desktop = apply_filters( 'cagov_desktop_media_query', '(min-width:1025px)' );
	$tablet_only = apply_filters( 'cagov_tablet_media_query', '(min-width: 769px) and (max-width: 1024px)' );
	$mobile = apply_filters( 'cagov_mobile_media_query', '(max-width:768px)' );
	$mobile_menu = apply_filters( 'cagov_mobile_menu_media_query', $mobile );
 ...
 */
function cagov_do_element_classes( $context, $class = '' ) {
	$after = apply_filters( 'cagov_after_element_class_attribute', '', $context );

	if ( $after ) {
		$after = ' ' . $after;
	}

	echo 'class="' . join( ' ', cagov_get_element_classes( $context, $class ) ) . '"' . $after; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in function.
}
 ...
 */
function cagov_get_element_classes( $context, $class = '' ) {
	$classes = array();
	...
	return apply_filters( "cagov_{$context}_class", $classes, $class );
}
 ...
 * @since 3.0.0
 */
function cagov_get_schema_type() {
	return apply_filters( 'cagov_schema_type', 'microdata' );
}
 ...
 */
function cagov_get_microdata( $context ) {
	$data = false;
	...
	if ( 'body' === $context ) {
		...
		$type = apply_filters( 'cagov_body_itemtype', $type );
	...
	if ( 'article' === $context ) {
		$type = apply_filters( 'cagov_article_itemtype', 'CreativeWork' );
	...
	if ( $data ) {
		return apply_filters( "cagov_{$context}_microdata", $data );
	}
}
 ...
 * @param string $context The element to target.
 */
function cagov_do_microdata( $context ) {
	echo cagov_get_microdata( $context ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in function.
}
 ...
 * @since 3.0.0
 */
function cagov_is_using_hatom() {
	return apply_filters( 'cagov_is_using_hatom', true );
}
 ...
 * @since 3.0.0
 */
function cagov_has_menu_bar_items() {
	return has_action( 'cagov_menu_bar_items' );
}
 ...
 */
function cagov_do_template_part( $template ) {
	/**
	 * cagov_before_do_template_part hook.
	 *
	 * @since 3.0.0
	 * @param string $template The template.
	 */
	do_action( 'cagov_before_do_template_part', $template );

	if ( apply_filters( 'cagov_do_template_part', true, $template ) ) {
	...
	/**
	 * cagov_after_do_template_parts hook.
	 *
	 * @since 3.0.0
	 * @param string $template The template.
	 */
	do_action( 'cagov_after_do_template_part', $template );
}
 ...
 * @since 3.0.0
 */
function cagov_has_inline_mobile_toggle() {
	$has_inline_mobile_toggle = cagov_is_using_flexbox() && ( 'nav-float-right' === cagov_get_navigation_location() || 'nav-float-left' === cagov_get_navigation_location() );

	return apply_filters( 'cagov_has_inline_mobile_toggle', $has_inline_mobile_toggle );
}
 ...
 * @since 3.0.0
 */
function cagov_get_the_title_parameters() {
	$params = array(
		'before' => sprintf(
	...
	return apply_filters( 'cagov_get_the_title_parameters', $params );
}
 ...
 * @since 3.0.0
 */
function cagov_has_default_loop() {
	return apply_filters( 'cagov_has_default_loop', true );
}
 ...
 * @since 3.0.0
 */
function cagov_needs_site_branding_container() {
	$container = false;

	if ( cagov_has_logo_site_branding() ) {
		if ( cagov_is_using_flexbox() || cagov_get_option( 'inline_logo_site_branding' ) ) {
			$container = true;
		}
	}
 ...
 *
 * The contextual filter is of the form `cagov_attr_{context}`.
 *
 * @since 3.1.0
 *
 ...
 */
function cagov_parse_attr( $context, $attributes = array(), $settings = array() ) {
	...
	$classes = cagov_get_element_classes( $context );
	...
	// Contextual filter.
	return apply_filters( 'cagov_parse_attr', $attributes, $context, $settings );
}

/**
 * Build list of attributes into a string and apply contextual filter on string.
 *
 * The contextual filter is of the form `cagov_attr_{context}_output`.
 *
 * @since 3.1.0
 *
 * @param string $context    The context, to build filter name.
 ...
 */
function cagov_get_attr( $context, $attributes = array(), $settings = array() ) {
	$attributes = cagov_parse_attr( $context, $attributes, $settings );
	...
	$output = apply_filters( 'cagov_get_attr_output', $output, $attributes, $context, $settings );
 ...
 * @param array  $settings   Optional. Custom data to pass to filter.
 */
function cagov_do_attr( $context, $attributes = array(), $settings = array() ) {
	echo cagov_get_attr( $context, $attributes, $settings ); // phpcs:ignore -- Escaping done in function.
}
 ...
 * @since 3.1.0
 */
function cagov_get_editor_color_palette() {
	$global_colors = cagov_get_option( 'global_colors' );
	$editor_palette = array();
	$static_colors = false;

	if ( apply_filters( 'cagov_color_palette_use_static_colors', false ) ) {
		$static_colors = true;
	}
 ...
 * @since 3.1.0
 */
function cagov_get_global_colors() {
	$global_colors = cagov_get_option( 'global_colors' );
	$colors = array();
 ...
 * @since 3.1.0
 */
function cagov_get_system_default_font() {
	return apply_filters( 'cagov_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
}
 ...
 * @since 3.1.0
 */
function cagov_has_active_menu() {
	$has_active_menu = true;

	if ( ! cagov_get_navigation_location() ) {
		$has_active_menu = false;
	}

	return apply_filters( 'cagov_has_active_menu', $has_active_menu );
}
 ...
 * @since 3.1.0
 */
function cagov_is_using_dynamic_typography() {
	return cagov_get_option( 'use_dynamic_typography' );
}
