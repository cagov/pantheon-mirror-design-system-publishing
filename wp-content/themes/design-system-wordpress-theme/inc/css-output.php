...
if ( ! function_exists( 'cagov_base_css' ) ) {
	/**
	 * Generate the CSS in the <head> section using the Theme Customizer.
	 *
	 * @since 0.1
	 */
	function cagov_base_css() {
		...
		$underline_links = cagov_get_option( 'underline_links' );
		...
		if ( apply_filters( 'cagov_do_group_inner_container_style', true ) ) {
		...
		$nav_drop_point = cagov_get_option( 'nav_drop_point' );
		$nav_location = cagov_get_navigation_location();

		if ( ( 'nav-float-right' === $nav_location || 'nav-float-left' === $nav_location ) && $nav_drop_point ) {
			$media_query = sprintf(
				'(max-width: %1$s) and %2$s',
				absint( $nav_drop_point ) . 'px',
				apply_filters( 'cagov_not_mobile_menu_media_query', '(min-width: 769px)' )
			);
			...
			$css->start_media_query( $media_query );
				...
				if ( 'nav-float-left' === cagov_get_option( 'nav_position_setting' ) ) {
		...
		if ( cagov_get_option( 'logo_width' ) ) {
			$css->set_selector( '.site-header .header-image' );
			$css->add_property( 'width', absint( cagov_get_option( 'logo_width' ) ), false, 'px' );
		}

		if ( cagov_get_option( 'back_to_top' ) ) {
		...
		if ( 'click' === cagov_get_option( 'nav_dropdown_type' ) || 'click-arrow' === cagov_get_option( 'nav_dropdown_type' ) ) {
		...
		$global_colors = cagov_get_global_colors();
		...
		do_action( 'cagov_base_css', $css );

		return apply_filters( 'cagov_base_css_output', $css->css_output() );
	}
}

if ( ! function_exists( 'cagov_advanced_css' ) ) {
	/**
	 * Generate the CSS in the <head> section using the Theme Customizer.
	 *
	 * @since 0.1
	 */
	function cagov_advanced_css() {
		$settings = wp_parse_args(
			get_option( 'cagov_settings', array() ),
			cagov_get_color_defaults()
		);
		...
		$css->start_media_query( cagov_get_media_query( 'mobile-menu' ) );
		...
		do_action( 'cagov_colors_css', $css );

		return apply_filters( 'cagov_colors_css_output', $css->css_output() );
	}
}

if ( ! function_exists( 'cagov_font_css' ) ) {
	/**
	 * Generate the CSS in the <head> section using the Theme Customizer.
	 *
	 * @since 0.1
	 */
	function cagov_font_css() {

		$settings = wp_parse_args(
			get_option( 'cagov_settings', array() ),
			cagov_get_default_fonts()
		);

		$defaults = cagov_get_default_fonts( false );

		$css = new GeneratePress_CSS();

		$body_family = cagov_get_font_family_css( 'font_body', 'cagov_settings', cagov_get_default_fonts() );
		$top_bar_family = cagov_get_font_family_css( 'font_top_bar', 'cagov_settings', cagov_get_default_fonts() );
		$site_title_family = cagov_get_font_family_css( 'font_site_title', 'cagov_settings', cagov_get_default_fonts() );
		$site_tagline_family = cagov_get_font_family_css( 'font_site_tagline', 'cagov_settings', cagov_get_default_fonts() );
		$navigation_family = cagov_get_font_family_css( 'font_navigation', 'cagov_settings', cagov_get_default_fonts() );
		$widget_family = cagov_get_font_family_css( 'font_widget_title', 'cagov_settings', cagov_get_default_fonts() );
		$h1_family = cagov_get_font_family_css( 'font_heading_1', 'cagov_settings', cagov_get_default_fonts() );
		$h2_family = cagov_get_font_family_css( 'font_heading_2', 'cagov_settings', cagov_get_default_fonts() );
		$h3_family = cagov_get_font_family_css( 'font_heading_3', 'cagov_settings', cagov_get_default_fonts() );
		$h4_family = cagov_get_font_family_css( 'font_heading_4', 'cagov_settings', cagov_get_default_fonts() );
		$h5_family = cagov_get_font_family_css( 'font_heading_5', 'cagov_settings', cagov_get_default_fonts() );
		$h6_family = cagov_get_font_family_css( 'font_heading_6', 'cagov_settings', cagov_get_default_fonts() );
		$footer_family = cagov_get_font_family_css( 'font_footer', 'cagov_settings', cagov_get_default_fonts() );
		$buttons_family = cagov_get_font_family_css( 'font_buttons', 'cagov_settings', cagov_get_default_fonts() );
		...
		if ( apply_filters( 'cagov_do_wp_block_margin_bottom', true ) ) {
		...
		$css->start_media_query( cagov_get_media_query( 'mobile' ) );
		$css->set_selector( '.main-title' );
		...
		do_action( 'cagov_typography_css', $css );

		return apply_filters( 'cagov_typography_css_output', $css->css_output() );
	}
}

if ( ! function_exists( 'cagov_spacing_css' ) ) {
	/**
	 * Write our dynamic CSS.
	 *
	 * @since 0.1
	 */
	function cagov_spacing_css() {
		$settings = wp_parse_args(
			get_option( 'cagov_spacing_settings', array() ),
			cagov_spacing_get_defaults()
		);

		$defaults = cagov_spacing_get_defaults( false );
		$sidebar_layout = cagov_get_layout();

		$css = new GeneratePress_CSS();

		$css->set_selector( '.inside-top-bar' );
		$css->add_property( 'padding', cagov_padding_css( $settings['top_bar_top'], $settings['top_bar_right'], $settings['top_bar_bottom'], $settings['top_bar_left'] ), cagov_padding_css( $defaults['top_bar_top'], $defaults['top_bar_right'], $defaults['top_bar_bottom'], $defaults['top_bar_left'] ) );

		if ( cagov_is_using_flexbox() && 'boxes' === cagov_get_option( 'container_alignment' ) ) {
			$top_bar_padding = absint( $settings['top_bar_right'] ) + absint( $settings['top_bar_left'] );

			$css->set_selector( '.inside-top-bar.grid-container' );
			$css->add_property( 'max-width', cagov_get_option( 'container_width' ) + $top_bar_padding, false, 'px' );
		}

		$css->set_selector( '.inside-header' );
		$css->add_property( 'padding', cagov_padding_css( $settings['header_top'], $settings['header_right'], $settings['header_bottom'], $settings['header_left'] ), cagov_padding_css( $defaults['header_top'], $defaults['header_right'], $defaults['header_bottom'], $defaults['header_left'] ) );

		if ( cagov_is_using_flexbox() ) {
			if ( 'boxes' === cagov_get_option( 'container_alignment' ) ) {
				$header_padding = absint( $settings['header_right'] ) + absint( $settings['header_left'] );

				$css->set_selector( '.inside-header.grid-container' );
				$css->add_property( 'max-width', cagov_get_option( 'container_width' ) + $header_padding, false, 'px' );
			}

			if ( 'text' === cagov_get_option( 'container_alignment' ) ) {
				...
				$css->add_property( 'padding', cagov_padding_css( 0, $navigation_right_padding, 0, $navigation_left_padding ) );
			}
		}
		...
		$css->add_property( 'padding', cagov_padding_css( $settings['content_top'], $settings['content_right'], $settings['content_bottom'], $settings['content_left'] ), cagov_padding_css( $defaults['content_top'], $defaults['content_right'], $defaults['content_bottom'], $defaults['content_left'] ) );

		if ( apply_filters( 'cagov_do_group_inner_container_style', true ) ) {
			$css->set_selector( '.site-main .wp-block-group__inner-container' );
			$css->add_property( 'padding', cagov_padding_css( $settings['content_top'], $settings['content_right'], $settings['content_bottom'], $settings['content_left'] ) );
		}
		...
		if ( ! cagov_is_using_flexbox() && 'text' === cagov_get_option( 'container_alignment' ) ) {
			$css->set_selector( '.container.grid-container' );
			$css->add_property( 'max-width', cagov_get_option( 'container_width' ) + $content_padding, false, 'px' );
		}
		...
		$css->add_property( 'margin', cagov_padding_css( '0', $settings['content_right'], '0', $settings['content_left'] ), cagov_padding_css( '0', $defaults['content_right'], '0', $defaults['content_left'] ) );
		...
		$css->set_selector( '.main-navigation .main-nav ul ul li a' );
		$css->add_property( 'padding', cagov_padding_css( $settings['sub_menu_item_height'], $settings['menu_item'], $settings['sub_menu_item_height'], $settings['menu_item'] ), cagov_padding_css( $defaults['sub_menu_item_height'], $defaults['menu_item'], $defaults['sub_menu_item_height'], $defaults['menu_item'] ) );
		...
		$css->add_property( 'padding', cagov_padding_css( $settings['widget_top'], $settings['widget_right'], $settings['widget_bottom'], $settings['widget_left'] ), cagov_padding_css( $defaults['widget_top'], $defaults['widget_right'], $defaults['widget_bottom'], $defaults['widget_left'] ) );

		if ( cagov_is_using_flexbox() ) {
			$css->set_selector( '.footer-widgets-container' );
			$css->add_property( 'padding', cagov_padding_css( $settings['footer_widget_container_top'], $settings['footer_widget_container_right'], $settings['footer_widget_container_bottom'], $settings['footer_widget_container_left'] ), cagov_padding_css( $defaults['footer_widget_container_top'], $defaults['footer_widget_container_right'], $defaults['footer_widget_container_bottom'], $defaults['footer_widget_container_left'] ) );
			...
			if ( 'boxes' === cagov_get_option( 'container_alignment' ) ) {
				...
				$css->add_property( 'max-width', cagov_get_option( 'container_width' ) + $footer_widgets_padding, false, 'px' );
			}
		} else {
			$css->set_selector( '.footer-widgets' );
			$css->add_property( 'padding', cagov_padding_css( $settings['footer_widget_container_top'], $settings['footer_widget_container_right'], $settings['footer_widget_container_bottom'], $settings['footer_widget_container_left'] ), cagov_padding_css( $defaults['footer_widget_container_top'], $defaults['footer_widget_container_right'], $defaults['footer_widget_container_bottom'], $defaults['footer_widget_container_left'] ) );
		...
		$css->add_property( 'padding', cagov_padding_css( '0', '0', '0', $settings['footer_widget_separator'] ), cagov_padding_css( '0', '0', '0', $defaults['footer_widget_separator'] ) );
		...
		if ( cagov_is_using_flexbox() ) {
			...
			$css->add_property( 'padding', cagov_padding_css( $settings['footer_top'], $settings['footer_right'], $settings['footer_bottom'], $settings['footer_left'] ), cagov_padding_css( $defaults['footer_top'], $defaults['footer_right'], $defaults['footer_bottom'], $defaults['footer_left'] ) );
			...
			if ( 'boxes' === cagov_get_option( 'container_alignment' ) ) {
				...
				$css->add_property( 'max-width', cagov_get_option( 'container_width' ) + $site_info_padding, false, 'px' );
			}
		} else {
			$css->set_selector( '.site-info' );
			$css->add_property( 'padding', cagov_padding_css( $settings['footer_top'], $settings['footer_right'], $settings['footer_bottom'], $settings['footer_left'] ), cagov_padding_css( $defaults['footer_top'], $defaults['footer_right'], $defaults['footer_bottom'], $defaults['footer_left'] ) );
		...
		$css->start_media_query( cagov_get_media_query( 'mobile' ) );
		$css->set_selector( '.separate-containers .inside-article, .separate-containers .comments-area, .separate-containers .page-header, .separate-containers .paging-navigation, .one-container .site-content, .inside-page-header' );
		$css->add_property( 'padding', cagov_padding_css( $settings['mobile_content_top'], $settings['mobile_content_right'], $settings['mobile_content_bottom'], $settings['mobile_content_left'] ) );
		...
		if ( ! cagov_is_using_flexbox() ) {
			...
			$sidebar = strtoupper( $cagov_settings['sidebar_widget_background_color'] );
			$content = strtoupper( $cagov_settings['content_background_color'] );
		...
		do_action( 'cagov_spacing_css', $css );

		return apply_filters( 'cagov_spacing_css_output', $css->css_output() . $output );
	}
}
 ...
 * @since 2.0
 */
function cagov_no_cache_dynamic_css() {
	$css = new GeneratePress_CSS();

	if ( cagov_is_using_flexbox() ) {
		$right_sidebar_width = apply_filters( 'cagov_right_sidebar_width', '30' );
		$left_sidebar_width = apply_filters( 'cagov_left_sidebar_width', '30' );
 ...
 * @since 3.0.0
 */
function cagov_get_dynamic_css() {
	if ( cagov_is_using_dynamic_typography() ) {
		$typography_css = GeneratePress_Typography::get_css();
	} else {
		$typography_css = cagov_font_css();
	}

	$css = cagov_base_css() . $typography_css . cagov_advanced_css() . cagov_spacing_css();

	return apply_filters( 'cagov_dynamic_css', $css );
}

add_action( 'wp_enqueue_scripts', 'cagov_enqueue_dynamic_css', 50 );
/**
 * Enqueue our dynamic CSS.
 *
 * @since 2.0
 */
function cagov_enqueue_dynamic_css() {
	if ( apply_filters( 'cagov_using_dynamic_css_external_file', false ) ) {
		$css = '';
	} elseif ( ! get_option( 'cagov_dynamic_css_output', false ) || is_customize_preview() || apply_filters( 'cagov_dynamic_css_skip_cache', false ) ) {
		$css = cagov_get_dynamic_css();
	} else {
		$css = get_option( 'cagov_dynamic_css_output' ) . '/* End cached CSS */';
	}

	$css = $css . cagov_no_cache_dynamic_css();
